...
def accept_incoming_connections():
    ...
    while True:
        ...
        if msg.decode("utf-8") == "Cluster":
          ...
          Thread(target = ClusterProccess, args=(client,)).start()
        ...
        elif msg.decode("utf-8") == "Cliente":
          
          clientes[client]=client_address
          Thread(target = Client, args=(client,)).start()


        
              
def ClusterProccess(client):

     if not len(addresses)<3:
       
          vidcap = cv2.VideoCapture('VideoCluster.mp4')
          ...
          for sock in clientes:
            file = open("videoBlancoNegro.mp4", "rb")
...
def Client(client):
  ...
  Video = client.recv(BUFSIZ)

  print("Soy cliente")
  file = open("VideoCluster.mp4", "wb")
  file.write(Video)
  file.close()
